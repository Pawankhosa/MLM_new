...
public partial class Admin_add_slider : System.Web.UI.Page
{
    ...
    protected void btnsubmit_Click(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["id"] != null)
        {
            objsql.ExecuteNonQuery("update tblslider set name='" + txtname.Text + "',image='" + imagename + "' where id='" + Request.QueryString["id"] + "'");
        }
        else
        {
            objsql.ExecuteNonQuery("insert into tblslider(name,image) values ('" + txtname.Text + "','" + imagename + "')");
    ...
    protected void bind(int id)
    {
        ...
        dt1 = objsql.GetTable("select * from tblslider where id='" + id + "'");
        if (dt1.Rows.Count > 0)
        {
