...
public partial class Admin_add_gallery : System.Web.UI.Page
{
    ...
    protected void btnsubmit_Click(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["id"] != null)
        {
            objsql.ExecuteNonQuery("update tblgallery set name='" + txtname.Text + "',image='" + imagename + "' where id='" + Request.QueryString["id"] + "'");
        }
        else
        {
            objsql.ExecuteNonQuery("insert into tblgallery(name,image,code) values ('" + txtname.Text + "','" + imagename + "','"+Request.QueryString["add"]+"')");
    ...
    protected void bind(int id)
    {
        ...
        dt1 = objsql.GetTable("select * from tblgallery where id='" + id + "'");
        if (dt1.Rows.Count > 0)
        {
